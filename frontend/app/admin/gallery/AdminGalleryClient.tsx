    ...
    try {
      ...
      if (eventFiles) {
        for (let i = 0; i < eventFiles.length; i++) {
          form.append('images', eventFiles[i]);
        }
      }
      const res = await fetch(`${backendBase}/api/admin/gallery/events`, {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 auto-rows-fr">
          ...
          <div className="lg:col-span-2 rounded-2xl bg-gray-900 p-6 shadow-lg flex flex-col justify-center">
            <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2">Gallery</h3>
          ...
          <>
            ...
            <div className="lg:col-span-2 lg:row-span-2 rounded-2xl bg-white border border-gray-200 shadow-lg p-6">
              ...
              <p className="text-sm text-gray-500 mb-4">
                New events appear on the public gallery.
              </p>
              {eventMessage && (
                <div
                  className={`mb-4 p-3 rounded-xl text-sm ${
                    eventMessage.type === 'success' ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'
                  }`}
                >
                  {eventMessage.text}
                </div>
            ...
            <div className="lg:col-span-2 rounded-2xl bg-gray-800 border border-gray-700 shadow-lg p-6">
              <h3 className="text-lg font-bold text-white mb-4">All events</h3>
              {eventsLoading ? (
                <p className="text-gray-400 text-sm">Loading…</p>
              ) : events.length === 0 ? (
                <p className="text-gray-500 text-sm">No events yet. Create one in the form.</p>
              ) : (
                <div className="overflow-x-auto -mx-2">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-600 text-left">
                        <th className="py-2 pr-3 font-semibold text-gray-400">Title</th>
                        <th className="py-2 pr-3 font-semibold text-gray-400">Date</th>
                        <th className="py-2 pr-3 font-semibold text-gray-400">Status</th>
                        <th className="py-2 pr-3 font-semibold text-gray-400">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {events.map((ev) => (
                          ...
                          <td className="py-2 flex gap-2">
                            ...
                            {deleteConfirmId === ev.id ? (
                              <>
                                ...
                                <button type="button" onClick={() => handleDeleteEvent(ev.id)} disabled={deleteLoading} className="text-red-400 hover:text-red-300">Yes</button>
                                <button type="button" onClick={() => setDeleteConfirmId(null)} className="text-gray-400">No</button>
                              </>
                            ) : (
                              <button type="button" onClick={() => setDeleteConfirmId(ev.id)} className="text-red-400 hover:text-red-300">Delete</button>
                            )}
                          </td>
                        </tr>
                      ))}
            ...
            {/* Bento: Allowed emails - spans 2 cols */}
            <div className="lg:col-span-2 rounded-2xl bg-gray-200 border border-gray-300 shadow-lg p-6">
              <h3 className="text-lg font-bold text-gray-900 mb-1">Allowed admin emails</h3>
              <p className="text-sm text-gray-600 mb-4">
                Who can sign in. You cannot remove the last email.
              </p>
              {emailsError && (
